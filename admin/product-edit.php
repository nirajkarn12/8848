...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
		...
		try {
			...
			$commissionStmt->execute(array($staffCommissionType, $staffCommissionValue, $_REQUEST['id']));
...
foreach ($result as $row) {
	...
	$ecat_id = $row['ecat_id'];
	$staff_commission_type = $row['staff_commission_type'] ?? 'inherit';
	$staff_commission_value = $row['staff_commission_value'] ?? 0;
}
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
								<select name="staff_commission_type" class="form-control">
									<option value="inherit" <?php echo (($staff_commission_type ?? 'inherit') === 'inherit') ? 'selected' : ''; ?>>Inherit (staff/global)</option>
									<option value="percent" <?php echo (($staff_commission_type ?? '') === 'percent') ? 'selected' : ''; ?>>Percentage</option>
									<option value="fixed" <?php echo (($staff_commission_type ?? '') === 'fixed') ? 'selected' : ''; ?>>Fixed Amount</option>
								</select>
							</div>
							<div class="col-sm-2">
								<input type="number" step="0.01" min="0" class="form-control" name="staff_commission_value" placeholder="Value" value="<?php echo htmlspecialchars($staff_commission_value ?? '0'); ?>">
							</div>
							<div class="col-sm-3">
